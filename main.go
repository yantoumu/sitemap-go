...
func main() {
	...
	defaultAPIWorkers := getEnvIntOrDefault("API_WORKERS", 2)
	...
	var (
		...
		batchSize     = flag.Int("batch-size", defaultBatchSize, "Backend submission batch size (env: BATCH_SIZE)")
...
func printUsage() {
	...
	fmt.Println("      API_WORKERS: 2")
	fmt.Println("")
	fmt.Println("PERFORMANCE OPTIMIZED:")
	fmt.Println("- Fast sitemap processing: 15 concurrent workers")
	fmt.Println("- Controlled API queries: 2 workers, 1 req/sec (avoid rate limits)")
